...
# Usage : bash evals/filet/rejouer.sh [plafond=6] [tag=filet] — témoin : bash evals/filet/rejouer.sh 80
...
LOG="$(mktemp)"
(cd "$K" && claude plugin eval . --tag "$TAG" --runs 1 --ablation none --no-publish --scaffold \
  --allow-tools Bash --trust-plugin --max-cost-usd 0.5 -j 1 --keep-temp \
  --json "$SRC/evals/results/$TAG-$N.json") 2>&1 | tee "$LOG"
