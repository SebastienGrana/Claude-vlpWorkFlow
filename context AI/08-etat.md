...
| 59 | `GAR` — Le gardien renvoie pour de vrai | `CON` a branché `vlp.py gardien` sur `SubagentStop` ; son renvoi n'a tiré que sur la sonde de `CON3`, jamais sur un vrai statut manquant, une case vide ou un commit qui nomme la fiche (le témoin de `CON5` a rendu `RETOUR` en tête). Deux témoins de plus : un qui finit par « Parfait », un qui rend `FAITE` sans cocher ; lire `Stop hook feedback` dans leur transcription. À jouer avec `ENR` (n° 56), qui monte déjà un vrai `/vlp:enchainer`. Même chantier : retirer `vlp.py sonde`, hook d'essai resté débranché. 🟡 Le renvoi sur `TÊTE` n'est testé qu'en unitaire. | ~1 fiche, plus l'essai | — |
