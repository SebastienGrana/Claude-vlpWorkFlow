...
| 48 | `REV` — Relire une fiche de code avant son commit | Le contrat de `/vlp:enchainer` commite un `FAITE` à case cochée sans relire le code (`skills/enchainer/SKILL.md:68-72`). Les trois fiches de code de `FIN` (2026-09-24) ont rendu `FAITE` sur du code à reprendre : la fin de plage en `max` au lieu de `min` et des tests creux (`FIN1`), un double compte et le test de bout en bout remplacé par un creux (`FIN2`), des gardes imprimées « GARDE: GARDE: » (`FIN3`). Même suite pour `TAR1` à `TAR3`, `PLA1` et `PLA2` : huit fiches de code sur huit reprises cette nuit-là. Suivi à la lettre, le contrat les aurait commités ; chaque reprise du chef s'est prouvée par un mutant. 🟡 À trancher au cadrage : le chef relit le diff et rejoue le mutant sur toute fiche de code, ou ne lit que la sortie du mutant que le critère nomme (`methode-chantier.md`, « Anatomie d'une fiche »). 🟡 Le coût d'une revue seule : pas mesuré. | ~2 fiches | — |
| 49 | `CON` — Le contrat du sous-agent, vérifié à sa sortie | Onze sous-agents la nuit du 2026-09-24 (`CAS1` à `PLA2`). Statut pas en tête 3 fois : `VAL1`, `FIN1`, `FIN2` ont mis leur `FAITE` en dernière ligne (12e, 27e, 11e), rattrapé par l'étape 2 de `/vlp:enchainer`. `FAITE` sur case vide 2 fois (`VAL1`, `FIN1`), rattrapé par `CAS`. Commit par le sous-agent 3 fois : `CAS1`, avant la règle ; `VAL1`, par `git -C`, la règle « aucun commit » écrite dans `agents/fiche.md` mais pas encore chargée — elle attend `/reload-plugins` ; `PLA1`, par `git add -A` : sa transcription porte 0 fois « aucun commit » et 4 fois « commit par tâche » (le `CLAUDE.md` global). 💡 Un hook `SubagentStop` qui renvoie le sous-agent tant que le mot n'est pas en tête ou la case vide. 🟡 Qu'un tel hook puisse le renvoyer : pas vérifié. Mesurer d'abord la règle « aucun commit » une fois chargée. | ~1 fiche | — |
