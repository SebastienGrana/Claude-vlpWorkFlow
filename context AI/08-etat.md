...
|---|---|---|---|---|
| 48 | `REV` — Relire une fiche de code avant son commit | Le contrat de `/vlp:enchainer` commite un `FAITE` à case cochée sans relire le code (`skills/enchainer/SKILL.md:68-72`). La nuit du 2026-09-24, de `FIN1` à `CAD1`, dix fiches de code sur dix ont rendu `FAITE` sur du code à reprendre — `FIN1` à `FIN3`, `TAR1` à `TAR3`, `PLA1`, `PLA2`, `MTK1`, `CAD1` : fin de plage en `max` au lieu de `min`, double compte, tests creux ou relâchés, gardes imprimées deux fois ; et `MTK2`, de doc, aussi — avant elles, `VAL1` aussi, repris par le chef. Suivi à la lettre, le contrat les aurait commités ; chaque reprise du chef s'est prouvée par un mutant. La revue ne suffit pas seule : `FIN`, repris et prouvé, a fait tomber à 0 le coût de dix vieux chantiers — trouvé par un `cout` sur les vrais fichiers, corrigé par `ZER`. 🟡 À trancher au cadrage : le chef relit le diff et rejoue le mutant sur toute fiche de code, ou ne lit que la sortie du mutant que le critère nomme (`methode-chantier.md`, « Anatomie d'une fiche ») ; et s'il rejoue la commande sur les fichiers réels. Mesuré la nuit du 2026-09-24 (`mesurer` sur la session principale, du retour de chaque sous-agent au commit suivant — relecture, reprise, mutants, journal, page, commit) : 177 tours · 40 990 843 tokens · 16,51 $ sur 14 fiches, contre 398 tours · 18 782 801 · 4,34 $ pour les 14 sous-agents — médiane 5,6 fois le sous-agent par fiche, de 1,3 à 8,9. La reprise coûte plus que le travail repris. 🟡 La part de la seule relecture, hors reprise et cérémonie : pas isolée ; la plus basse, `FIN3`, 5 tours · 0,44 $. | ~2 fiches | — |
...
| 39 | `PYT` — Plus d'erreur « Python est introuvable » à chaque appel d'outil | Sous Windows, les entrées `python3` de `hooks/hooks.json` échouent à chaque appel d'outil : une `hook_non_blocking_error` « Python est introuvable » par appel, deux par écriture. Compté la nuit du 2026-09-24 : 598 dans la session principale (73,1 s) et 488 dans les quatorze sous-agents (60,1 s), toutes de l'entrée `python3`. Aucune n'a paru dans le contexte du modèle de la session principale ; pour les sous-agents, 🟡 déduit, pas vérifié. Le coût : ≈ 133 s et du bruit à l'écran, pas des tokens. Faire taire l'entrée qui échoue sans perdre la paire `python3` + `py`. | ~1 fiche | — |
