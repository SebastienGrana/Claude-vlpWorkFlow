...
| 44 | `GLO` — Le `CLAUDE.md` de l'utilisateur dans le sous-agent | Vu dans `FIL3` : `vlp:fiche` reçoit le `CLAUDE.md` global de l'utilisateur (attachment `instructions`) et en suit la forme — son dernier message finit par la jauge « ⚠️ Imprévu — … ». Le déplacement est vu depuis : `FIN2` (2026-09-24) ouvre son compte rendu par « ## En résumé » et la jauge « ✅ **Tout va bien** », son `FAITE` en 11e et dernière ligne — rattrapé par l'étape 2 de `/vlp:enchainer`. 🟡 Ce qu'il pèse par sous-agent : pas mesuré. Mesurer d'abord ; n'agir que si ça coûte. | ~0,5 fiche | — |
...
| 48 | `REV` — Relire une fiche de code avant son commit | Le contrat de `/vlp:enchainer` commite un `FAITE` à case cochée sans relire le code (`skills/enchainer/SKILL.md:68-72`). Les trois fiches de code de `FIN` (2026-09-24) ont rendu `FAITE` sur du code à reprendre : la fin de plage en `max` au lieu de `min` et des tests creux (`FIN1`), un double compte et le test de bout en bout remplacé par un creux (`FIN2`), des gardes imprimées « GARDE: GARDE: » (`FIN3`). Suivi à la lettre, le contrat les aurait commités ; chaque reprise du chef s'est prouvée par un mutant. 🟡 À trancher au cadrage : le chef relit le diff et rejoue le mutant sur toute fiche de code, ou ne lit que la sortie du mutant que le critère nomme (`methode-chantier.md`, « Anatomie d'une fiche »). 🟡 Le coût d'une revue seule : pas mesuré. | ~2 fiches | — |
| 49 | `CON` — Le contrat du sous-agent, vérifié à sa sortie | Six sous-agents la nuit du 2026-09-24 (`CAS1` à `FIN3`). Statut pas en tête 3 fois : `VAL1`, `FIN1`, `FIN2` ont mis leur `FAITE` en dernière ligne (12e, 27e, 11e), rattrapé par l'étape 2 de `/vlp:enchainer`. `FAITE` sur case vide 2 fois (`VAL1`, `FIN1`), rattrapé par `CAS`. Commit par le sous-agent 2 fois : `CAS1`, avant la règle ; `VAL1`, par `git -C`, la règle « aucun commit » écrite dans `agents/fiche.md` mais pas encore chargée — elle attend `/reload-plugins`. 💡 Un hook `SubagentStop` qui renvoie le sous-agent tant que le mot n'est pas en tête ou la case vide. 🟡 Qu'un tel hook puisse le renvoyer : pas vérifié. Mesurer d'abord la règle « aucun commit » une fois chargée. | ~1 fiche | — |
| 50 | `CAD` — Le cadrage dans le coût du chantier | Seul `cocher` pose une ligne `**Session**` ; `/vlp:chantier` n'en pose aucune. Un cadrage joué dans sa session, puis `/clear` (« une tâche, une session »), n'entre dans aucun total. Il pèse : celui de `VAL`, mesuré par plage, fait 2 087 345 tokens · 7 tours, pour une fiche à 3 368 752 (`45-cout-aux-bords.md`). `ouvrir` notera la session du cadrage, et `cout` la comptera dans hors fiches. Exclu de `FIN` (« Ce qu'on ne fait pas »). | ~1 fiche | — |
| 51 | `PLA` — La plage des fiches suit le fichier | L'en-tête d'une page de chantier (« fiches X1–Xn ») ne s'écrit qu'à `page --creer` (`scripts/vlp.py:1153`) : ni `page` ni `clore` n'y touchent. La ligne d'index d'un chantier ouvert ne s'écrit qu'une fois (`scripts/vlp.py:2122`) — `clore` la refait ; `ouvrir` relancé ne la touche pas (`index +0`, mesuré à l'ouverture de `PLA`). Vu sur `CAS` (2026-09-24) : `CAS2` ajoutée après l'ouverture, index et en-tête restés à `CAS1`, corrigés à la main. `page` réécrira l'en-tête depuis le fichier de fiches, et la ligne d'index d'un chantier ouvert, test à l'appui. | ~0,5 fiche | — |
