...
2026-09-24 ; 53, du cadrage de `REV` ; 54, d'une demande de l'utilisateur pendant `REV5` ; 55 à 57, de la clôture de `REV` ; 58, d'une demande de l'utilisateur le 2026-09-25 ; 59 à 61, de la clôture de `CON` ; 66, de celle de `JUG` ; 67, de celle de `REC` ; 68, d'`ESS4` ; 69, d'une demande de l'utilisateur pendant `LEC2` ; 70, de `REL1`.
...
| 56 | `ENR` — La relecture dans un vrai `/vlp:enchainer` | **Vu en vrai le 2026-09-25, sans piège exprès** (chantier `ESD`) : `ESD2` `FAITE` puis `REFUSÉE` (mutant « ignorer la marque » vivant), chaîne arrêtée, page marquée bloquée ; `ESD3` `ACCEPTÉE` puis commitée. Reste à trancher s'il faut encore un essai piégé. `REV3` a branché `vlp:relire` dans `/vlp:enchainer`, mais la relecture n'a tourné qu'appelée à la main par le chef (`REV4`, 4 passages, 0,50 $ la relecture au dernier état). Jamais sur un `FAITE` vivant, ni avec un `REFUSÉE` qui arrête la chaîne et marque la page bloquée. Un chantier de deux ou trois fiches enchaîné, dont une piégée, le prouvera. | ~1 fiche, plus l'essai | — |
