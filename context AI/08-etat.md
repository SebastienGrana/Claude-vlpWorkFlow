...
| 59 | `GAR` — Le gardien renvoie pour de vrai | `CON` a branché `vlp.py gardien` sur `SubagentStop` ; son renvoi n'a tiré que sur la sonde de `CON3`, jamais sur un vrai statut manquant, une case vide ou un commit qui nomme la fiche (le témoin de `CON5` a rendu `RETOUR` en tête). Deux témoins de plus : un qui finit par « Parfait », un qui rend `FAITE` sans cocher ; lire `Stop hook feedback` dans leur transcription. À jouer avec `ENR` (n° 56), qui monte déjà un vrai `/vlp:enchainer`. Même chantier : retirer `vlp.py sonde`, hook d'essai resté débranché. Vu en vrai la nuit du 2026-09-25 (`GLO1`, transcription `agent-ac9db03f95e8ec2b0`) : le renvoi sur la tête a tiré — « Ton dernier message commence par « Excellent! » », deux fois (`python3` et `py`, cf. `PYT`) ; puis le sous-agent a rendu `FAITE` **sur case vide**, et rien ne l'a renvoyé : `cmd_gardien` ne juge plus un arrêt dès que `stop_hook_active` est vrai, donc le second arrêt passe sans contrôle. À corriger ici : juger la case et `TÊTE` même quand `stop_hook_active`, en gardant l'échappatoire `RETOUR`. 🟡 Le renvoi sur `TÊTE` n'est testé qu'en unitaire. | ~1 fiche, plus l'essai | — |
| 60 | `RLG` — Le gardien derrière le relecteur | `vlp.py gardien` ne tient que les sous-agents dont l'`agent_type` contient « fiche » : `vlp:relecture` lance `git` (worktrees, `diff`, `log`) sans que rien ne l'empêche d'écrire. Jamais vu arriver ; laissé « dehors » par le socle de `CON`. Étendre le refus de `PreToolUse` au relecteur : lire Git oui, `commit`/`add`/`reset` non — 🟡 sauf les `git worktree` que `vlp.py relecture` pose lui-même, à distinguer. Test : le cas `vlp:relecture` laissé passer de `CON4` devient refusé sur `commit`. | ~1 fiche | — |
