...
| 62 | `FOR` — La forme du sous-agent, pour de vrai | `GLO3` a écrit dans `agents/fiche.md` et `agents/relecture.md` : « Ton lecteur est le chef, pas l'humain » (règles de forme d'un `CLAUDE.md` exclues). Mesuré le 2026-09-25 par `vlp.py forme` sur les 14 sous-agents de la session `ca431cf8-…` écrits après `5e37964` : resume 7, jauge 10, tete 14 — avant `GLO3`, 14 et 23 sur 36 (journal `GLO2`). Tous les relecteurs portent la jauge (8 sur 8). La phrase n'a rien changé ; le gardien tient la tête, pas la fin. 🟡 Ce que la fin coûte : des lignes lues par le chef à chaque `Result`, et un « ⚠️ Imprévu » de relecteur pris pour une erreur du chef (mémoire). Pistes : couper au premier `---` côté chef, ou renvoyer par `SubagentStop`. Limite vue en chemin : `forme --depuis` filtre sur le départ de la session parente, et rate les sous-agents d'une longue session. | ~1 fiche | — |
