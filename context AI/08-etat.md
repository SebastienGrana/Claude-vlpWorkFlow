...
| 51 | `PLA` — La plage d'en-tête de la page suit le fichier | L'en-tête d'une page de chantier (« fiches X1–Xn ») ne s'écrit qu'à `page --creer` (`scripts/vlp.py:1153`) : ni `page` ni `clore` n'y touchent. Une fiche ajoutée après l'ouverture n'y apparaît jamais. `page` la réécrira depuis le fichier de fiches, test à l'appui. | ~0,5 fiche | — |
